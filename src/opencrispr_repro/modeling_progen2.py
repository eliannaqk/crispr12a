...
class ProGenAttention(nn.Module):
    ...
    def forward(
        ...
        if output_attentions:
            ...
            attn_output, attn_weights = self._sdpa_attn(query, key, value, attention_mask)
...
class ProGenForCausalLM(ProGenPreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        if labels is not None:
            ...
            # Flatten the tokens
            # Use pad token id (0) as ignore_index to match collate behavior,
            # where pads and the first token are set to 0.
